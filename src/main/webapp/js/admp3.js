	 $(document).ready(function() {
         //调用函数，初始化表格 
         var urlStr = "http://localhost:8080/getHandleCourse";
         TableInit(urlStr);
	...
	function checkChoose2() {
	 ...
	 function setNoPass() { //审核不通过
		 ...
		 if(a.length<=0){
			...
			var b=JSON.stringify(a);
			  var url="http://localhost:8080/setNoPassed";
	  		    $.ajax({
	  		        dataType: "json",
	  		        async: "false",
	  		        traditional:true,//这使json格式的字符不会被转码
	  		        data: {"datalist":b,"result":result},
	  		        type: "post", 
	  		        url: url,
	 ...
	 function setPass() { //排课提交
		 ...
		 if(a.length<=0){
			...
			var b=JSON.stringify(a);
			  var url="http://localhost:8080/setPassed";
	  		    $.ajax({
	  		        dataType: "json",
	  		        async: "false",
	  		        traditional:true,//这使json格式的字符不会被转码
